...
# calculate rms of difference between PINN and BiFrost as a function of height
rms_diff = np.std(mod - bif, axis=0)

# calculate rms of BiFrost parameters as a function of height
# rms = np.std(bif, axis=(0,1))
rms = np.std(bif, axis=0)
...
zend   = 50
for i in range(npar):
    # plt.plot(zaxis[zstart:zend] / 1000.0, rms_diff[zstart:zend,i] / rms[i], label = line_label[i], ls=line_style[i])
    plt.plot(zaxis[zstart:zend] / 1000.0, rms_diff[zstart:zend,i] / rms[zstart:zend,i], label = line_label[i], ls=line_style[i])
    # plt.plot(zaxis / 1000.0, rms[:,i], label = line_label[i], ls=line_style[i])
plt.legend(prop={'family':'Times New Roman'})
plt.xlabel('height [km]')
plt.ylabel('$\\sigma(PINN-Bifrost)/\\sigma(Bifrost)$')
